...
import data.qoutes;
import scanner.scannerbox;
import indicators.chart;
import watchlist.watchlistbox;
...
public class Graph extends JFrame implements ActionListener, MouseListener, MouseMotionListener {
    ...
    public Graph(String tick)
    {
        ...
        add(createButton("60 Min", offset, 60, 50));
        offset += space;
        add(createButton("30 Min", offset, 30, 30));
        offset += space;
        add(createButton("15 Min", offset, 15, 20));
        offset += space;
        add(createButton("5 Min", offset, 5, 10));
        offset += space;
        add(createButton("1 Min", offset, 1, 2));
    ...
    public void paint(Graphics g2) {
        ...
        if (newchart == true)
        {
            ...
            g.setFont(new Font("TimesRoman", Font.PLAIN, 13));
                ...
                g.drawString("Volume: 714000", 585, y);
    ...
    private boolean     is_crypto(String name)
    {
        boolean is_crypto;

        is_crypto = false;
        String  crypto_names[] = {"BTC", "ETH", "LTC", "XRP", "XMR", "IOTA", "BCH", "STR", "ZEC", "DASH", "ETC", "REP"};
        for (int i = 0; i < crypto_names.length; i++)
        {
            if (crypto_names[i].equals(name))
                is_crypto = true;
        }
        return (is_crypto);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (x > 70 && y < 500 && x < 970 && y > 50)
        {
            ...
            if (is_crypto(name)) {
                stuff = new qoutes(name);
            }
            else {
                 stuff = new qoutes(name, intervals, numberofdays);
            }
            data = stuff.smoothed();
            this.data = stuff;
            if (!data.isEmpty())
